...
async function getSigingKey(header:TokenHeader, next: any) {
...
function validateToken(token:string, event:Object){
    let signedKey:any = getSigingKey
   return new Promise((resolve, reject)=>{
 
       jwt.verify(token,signedKey,verificationOptions,(err:any, decoded:any)=>{
         if(err){
